...
if (isset($_POST['btnSave'])) {
	$txtfproductname = $_POST['txtfproductname'];
	$txtfprice = $_POST['txtfprice'];
	$txtfquantity = $_POST['txtfquantity'];
	$txtdes = $_POST['txtdes'];
	...
	$select = "SELECT * FROM FProduct WHERE FProductName='$txtfproductname'";
	$run = mysqli_query($connect, $select);
	$count = mysqli_num_rows($run);
	if ($count > 0) {
		echo "<script>window.alert('Product Name is already added!')</script>";
		echo "<script>window.location='FProductEntry.php'</script>";
		exit();
	} else {
		$insert = "INSERT INTO FProduct(FProductName,FPrice,FQuantity,Description,Image1,Image2,Image3,FCategoryID)
		values('$txtfproductname','$txtfprice','$txtfquantity','$txtdes','$fileImage1','$fileImage2','$fileImage3','$cbocategory')";
...
<body>
	<form action="FProductEntry.php" method="POST" enctype="multipart/form-data">
		<fieldset>
			...
			<table border="1px" align="center" cellpadding="20px">
				<tr>
					<td>Product Name</td>
					<td>
						<input type="text" name="txtfproductname" placeholder="Enter Product Name">
					</td>
				</tr>
				<tr>
					<td>Price</td>
					<td>
						<input type="number" name="txtfprice" placeholder="Enter Product Price">
					</td>
				</tr>
				<tr>
					<td>Quantity</td>
					<td>
						<input type="text" name="txtfquantity" placeholder="Enter Product Quantity">
					</td>
				</tr>
				<tr>
					<td>Description</td>
					...
					<td>
						<select name="cbocategory">
							...
							for ($i = 0; $i < $count; $i++) {
								...
								$categoryid = $row['FCategoryID'];
								$category = $row['FCategory'];
